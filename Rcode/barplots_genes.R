...
p1=ggplot() + labs(title = "Lake Champlain - all annotated species (ChampSt1 samples)",fill = "Taxonomy") +
  ...
  geom_bar(aes(y = fraction, x = sample, fill = species),
           data = all_spp_m_ggplot_top12_st1,stat="identity") + ylab("fraction of annotated genes") + scale_fill_manual(values = x(length(all_spp_five_m))) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
p2=ggplot() + labs(title = "Lake Champlain - all annotated species (ChampSt2 samples)",fill = "Taxonomy") +
  ...
  theme(plot.title = element_text(hjust = 0.5, size=14, face="bold")) +
  geom_bar(aes(y = fraction, x = sample, fill = species),
           data = all_spp_m_ggplot_top12_st2,stat="identity") + ylab("fraction of annotated genes") + scale_fill_manual(values = x(length(all_spp_five_m))) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
p3=ggplot() + labs(title = "Lake Champlain - all annotated species (ChampPRM samples)",fill = "Taxonomy") +
  ...
  theme(plot.title = element_text(hjust = 0.5, size=14, face="bold")) +
  geom_bar(aes(y = fraction, x = sample, fill = species),
           data = all_spp_m_ggplot_top12_PRM,stat="identity") + ylab("fraction of annotated genes") + scale_fill_manual(values = x(length(all_spp_five_m))) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
