...
system("ls -1 results/Metagenomic_Champlain_long_term/org_results/*RefSeq_annot_organism.tsv >all.tsv")
...
all_spp_five = c(all_spp_five,refseq.all[refseq.all[,2]>2,1]) #keep only the ones that are at more than 5%
}
#simplify by genus
...
for(i in 1:nrow(all.tsv))
{
  ...
  date = substring(shortname,nchar(shortname)-11,nchar(shortname)-4)
...
all_spp_m_ggplot = data.frame(unlist(all_spp_m[,2:35]))
temp = all_spp_m_ggplot
all_spp_m_ggplot[,2] = rep(all_spp_m[,1],34)
all_spp_m_ggplot[,3] = dmy(dates)
all_spp_m_ggplot[,4] = locations
all_spp_m_ggplot[,5] = factor(all_spp_m_ggplot[,4], levels=c('st1',"st2"))
...
p1=ggplot() + labs(title = "Lake Champlain - annotated genera - Long term",fill = "Taxonomy") +
  theme_bw() + 
...
dev.print(device=pdf,"figures/Metagenomic_Champlain_long_term_genera_barplot.pdf", onefile=FALSE)
dev.off()


png("figures/Metagenomic_Champlain_long_term_genera_barplot.png",width=10, res =400,height=7,units= 'in')
p1 + facet_grid(rows=vars(locations_f))
dev.off()
