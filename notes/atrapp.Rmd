...
#Data
• 2016 Lake Champlain metatranscriptomes (2 X 100bp Illumina HiSeq)  
...
paste(signif(mean(refseq[,1]),4),": mean nb sequence annotated (refseq)",sep = "")

#subsys
paste(min(subsys[,1]),": mininum nb sequence annotated (subsys)",sep = "")
paste(max(subsys[,1]),": maximum nb sequence annotated (subsys)",sep = "")
paste(signif(mean(subsys[,1]),4),": mean nb sequence annotated (subsys)",sep = "")
...
knitr::include_graphics("../figures/Champlain_barplot.png")
```
&nbsp;  
&nbsp;  
...
##*I. massiliense*  
• All *I. massiliense* 20160915_St1 sequences match to a single hypothetical (*i.e predicted*) protein (*WP_053955528.1*).  
...
##*B. massiliensis*  
• All *B. massiliensis* 20160915_St1 sequences match to a single hypothetical (*i.e predicted*) protein (WP_059086336.1).  
• *e-values* ~e-10 (because sequences are small + %identity ~90%)

###*B. massiliensis* genome BLAST
• Download *B. massiliensis* genome (3.5 Mbp).  
• BLAST all (for now 100k) sequences.  
• Sequences BLAST to **several** region of the genome.  
...
• Same conclusion: may not be *B. massiliensis*, but something close and more abundant in these samples
&nbsp;  
...
#running the barplot graph
#system("./Rcode/barplots.R")
knitr::include_graphics("../figures/Champlain_functions_barplot.png")
```
&nbsp;  
