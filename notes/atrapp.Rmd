...
paste(nb_seq,": Total Number of sequence (rough average)",sep = "")
...
```  
• Note: I can't remember the *Diamond* threshold (e^10?, check on cedar)...    
...
knitr::include_graphics("../figures/Champlain_species_barplot.pdf")
```
&nbsp;  
&nbsp;  
...
knitr::include_graphics("../figures/env_data.png")
```
&nbsp;  
&nbsp;
...
• Here I only show *spp.* that are present >2% in at least one of the samples.  
```{r functions, warning = F, message = F,out.width=1050,out.height=1200,fig.align='center',echo=F}
#running the barplot graph
#system("./Rcode/barplots.R")
knitr::include_graphics("../figures/Champlain_functions_barplot_level4.pdf")
```
&nbsp;  

##Principal Coordinate Analyses
• On the 25 most abundant species (note that this is preliminary as names will change to to a bug fis in Samsa2: bug explains differences btwn PRM and others...)
```{r pcoa, warning = F, message = F,out.width=500,out.height=600,fig.align='center',echo=F}
knitr::include_graphics("../figures/Champlain_pcoa.pdf")
```


#To do (lab meeting Oct 19th):
• What is *sp. PCC*?  
**bug in the SAMSA2 python annotation scripts: IN PROGRESS**

• Check the *Microcystis* in the database and in the list of annotated genes?  
• Get a *Microcystis* protein set (or predict it from genome): talk to Olga?  
**There are 67425 Microcystis gene products (63.5k are for M. aeruginosa)**

• Check illumina primer/adaptor sequences (find them on nanuq), if they may cause contamination (esp. for the 09-15 samples)? **done. Refiltered samples... oct 30th**
