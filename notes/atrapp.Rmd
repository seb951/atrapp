...
* Cyanotoxic genes rise similary to what was identified in metatranscriptomics (although more diversity is found here)
...
* First, you can see the difference in the bloom composition between the 2015 and 2016 metagenomics datasets (First 2 panels). In 2015, it is dominated by *Microcystis*. In 2016, it is dominated by *Dolichospermum*, but in September *Microcystis* start to show up and dominate.
* In 2016, for which we had metagenomics vs metatranscriptomics at similar dates, the *Dolichospermum* story is similar, but the *Microcystis* are almost absent from metatrasncriptomics... This means that they are there (according to metagenomics), but not active (according to metatranscriptomics).
* Overall, the metatranscriptomics is more diverse, while the metagenomics is largely dominated by cyanobacteria...
* I will compare *alpha* diversity between both datasets
* Maybe also Mantel tests to compare both data matrices.
...
* I've ran the pipeline of these samples (they are all St1 according to Nicholas).
* Species were grouped by genera to simplify it (e.g.: There were a lot of different *Flavobacterium* species)
* Altough this dataset is not comparable to the metatranscriptome directly, if I extrapolate it seems like there is little correlation btwn metat and metag.: This is pretty cool!
