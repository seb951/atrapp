...
* Identify *Dolichospermum* (main bloom causing cyanobacteria) genes that change in allele (SNPs) frequency from the beginning to the end of the bloom. These represent candidates genes whose changes in allele frequencies may help to predict blooms in the future.


## Method
1. Water sampled at three location in Lake Champlain (Baie Missisquoi: St1, St2, PRM, **see Figure 1**) during the cyanobacterial bloom in the summer of 2016 (June 1st - October 10th).
...
4. Sequences specifically annotated to *Dolichospermum* (the main cyanobacteria responsible for the bloom) used to produce a *de novo* reference transcriptome assembly using Trinity [@grabherr2011trinity]
5. *Dolichospermum* sequences aligned to transcriptome using bowtie2 [@langmead2012fast]. SNP called using SAMtools [@li2009sequence].
6. Changes in allele frequency quantified using regressions (logistic for St1 & Ste, linear for PRM) in R.
7. Candidate genes showing significant changes (FDR corrected *p*-value < 0.05) in allele frequency matched to reference database and compared against reference transcriptome.  
